...
$currentdatenumcount = date('Y-m-d');


 $data = mysql_query("SELECT * FROM wp_eme_events where event_start_date >='$currentdatenumcount'") or die(mysql_error()); 
...
// --------------------------Header loop ---------------------------------------------
 ...
 $headerresults = ("SELECT * FROM wp_eme_events WHERE event_start_date >= CURDATE() ORDER BY event_start_date ASC $max") or die(mysql_error()); 
...
while($row = mysql_fetch_array($testresult)){   //Creates a loop to loop through results
		...
		$eventdatetitle =  date('F d', strtotime($eventdate));
		$eventdatetitle2 =  date('F d Y', strtotime($eventdate));


$counter2++;
switch ($counter2)
{
	// case $counter2 == '0' : echo '<div class="eventtitle"><h2>Events - <span style="color:#ac0404;">'.$eventdatetitle;break;
    case $counter2 == '1' : echo '<div class="eventtitle"> <br><span class="eventdatestyle">'.$eventdatetitle;break; 
    // case $counter2 == '6'  : echo ' to '.$eventdatetitle2.'</span> '."( Page $pagenum of $last ) "; break; 
...
if (empty($eventdate)) {
	...
	echo "Sorry there are no current events listed at the current time, please check back soon...";
}
...
echo ' to '.$eventdatetitle2."</span><br><span class='eventpagenumstyle'>( Page $pagenum of $last )</span><br> ";
}else {
...
// First we check if we are on page one. If we are then we don't need a link to the previous page or the first page so we do nothing. If we aren't then we generate links to the first page, and to the previous page.
 ...
 echo " <a href='".get_permalink( $post->ID )."?pagenum=1'> <div class='paginationbuttons'>FIRST</div></a> ";
 echo " ";
 $previous = $pagenum-1;
 echo " <a href='".get_permalink( $post->ID )."?pagenum=$previous'> <div class='paginationbuttons'>PREV</div></a> ";
 } 
...
echo "<a href='".get_permalink( $post->ID )."?pagenum=$next'><div class='paginationbuttons'>NEXT</div></a> "; 
echo " <a href='".get_permalink( $post->ID )."?pagenum=$last'><div class='paginationbuttons'>LAST</div></a> ";
 } 
echo "<hr style='margin-top: 20px;font-size: 15px;'></div>";
}
...
// ("SELECT * FROM wp_eme_events ORDER BY event_start_date ASC $max")


//This is your query again, the same one... the only difference is we add $max into it
 $data_p = ("SELECT * FROM wp_eme_events WHERE event_start_date >= CURDATE() ORDER BY event_start_date ASC $max") or die(mysql_error()); 
...
$eventdescript = strip_tags($row['event_notes']);
...
<a href='/singleevent/?event_id=<?php echo $eventurl; ?>'>		
	<div class="eventboxwrapper">

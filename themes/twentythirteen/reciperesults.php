...
<br>
	...
	<form action="<?php the_permalink(); ?>" method="POST"> 
	 	<input type="text" name="search" size="<?php echo $length; ?>" placeholder="Search Results" value="<?php echo $searchboxtext; ?>">
		<span style="margin-left:15px;font-size:15px;color:#767676;">Type in box to search again. <?php echo $count.' Recipes found'; ?></span>
	</form>
...
<?php
  ...
  if (count($_SESSION['output'])) {
    ...
    $productPages = $pagination->generate($_SESSION['output'], 25);
    // If we have items 
    if (count($productPages) != 0) {
      ...
      foreach ($productPages as $value) {
        ...
        $result= mysql_query("SELECT * FROM `recipes` WHERE recipe_name LIKE '$value'");
			...
			echo '<div class="recipedate">'.$recipedate.'</div>';
			echo '</div></a>';

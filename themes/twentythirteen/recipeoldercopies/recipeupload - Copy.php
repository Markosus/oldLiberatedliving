...
if (isset($_SESSION['recipename'])){	
	...
	$query = "INSERT INTO recipes (recipe_name, linked_recipe_id,recipe_category, recipe_subcategory, recipe_main_ingredient,serving_size, date_added)
	          VALUES('{$_SESSION['recipename']}','{$_SESSION['linkedrecipe']}','{$_SESSION['maincategory']}','{$_SESSION['subcategory']}','{$_SESSION['mainingredient']}','{$_SESSION['servingsize']}',NOW())";
...
// inserts recipe ingredient
			...
			if (isset($_SESSION['name'])){
				...
				foreach($_SESSION['name'] as $value){
					...
					$query2 = "INSERT IGNORE INTO recipe_ingredients (ingredient_name)
          			VALUES('$value')";
					mysql_query($query2);
			...
			if (isset($_SESSION['decimalamt'])){
				...
				$counter=0;
				 foreach($_SESSION['decimalamt'] as $value){
					...
					$query3 = "INSERT INTO recipe_recipeingredients (recipe_id,ingredient_id,amount,unittype,actioninfo)
          			VALUES('$rec_id','$ingredient_id[$counter]','$value','{$_SESSION['amountunit'][$counter]}','{$_SESSION['ingredientaction'][$counter]}')";
...
// inserts the recipe description to the recipe if it exists
				 ...
				 if (!empty($_SESSION['recipedescription'])){	
				
						$querydescription = "INSERT INTO recipe_description (recipe_id,description)
		          		VALUES('$rec_id','{$_SESSION['recipedescription']}')";
...
<div id="titlediv">
	...
	<?php echo $_SESSION['linkedrecipe']; ?>
<hr />



		<a href="/addrecipe">Add Recipe</a><br>
		...
		<a href="/recipesearch">Search Recipes</a>
